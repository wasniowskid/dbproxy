...
public class LoggableStatement implements PreparedStatement {
	...
	private static final AtomicLong counterSequence = new AtomicLong(0L);
	private final static ConcurrentHashMap<Long, LoggableStatement> loggableStatements = new ConcurrentHashMap<>(200);
	...
	private void registerLoggableStatement() {
		counter = counterSequence.incrementAndGet();
	...
	static {
		Thread th = new Thread() {
			...
			public void run() {
				while (true) {
					Collection<LoggableStatement> statements = loggableStatements.values();
					LOG.info("Monitoring LoggableStatements... loggableStatements.size()={}", statements.size());
					long now = System.currentTimeMillis();

					for (LoggableStatement ls : statements) {
